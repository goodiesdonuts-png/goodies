...
type View = 'dashboard' | 'sales-points' | 'expenses' | 'point-detail' | 'reports' | 'pricing';
...
export default function App() {
    ...
    const [pricingProfiles, setPricingProfiles] = useState<any[]>(() => {
        ...
        return [{
            ...
            packagingItems: [
                { id: 'e1', name: 'Embalagem PlÃ¡stica', quantity: 1, totalValue: 0.20 }
            ],
            timeItems: [],
            fixedCosts: 1000,
    ...
    const [productionOrders, setProductionOrders] = useState<any[]>(() => {
        const saved = localStorage.getItem('beik-production-orders');
        return saved ? JSON.parse(saved) : [];
    });

    const handleGeneratePDF = () => {
        if (expiringSales.length === 0) {
            alert('Não há produtos próximos do vencimento para gerar ordem de produção.');
            return;
        }

        const grouped = expiringSales.reduce((acc, sale) => {
            const point = sale.point_name || 'Desconhecido';
        ...
        const updatedOrders = [newOrder, ...productionOrders];
        setProductionOrders(updatedOrders);
        localStorage.setItem('beik-production-orders', JSON.stringify(updatedOrders));
    ...
    useEffect(() => {
        localStorage.setItem('beik-pricing-profiles', JSON.stringify(pricingProfiles));
    }, [pricingProfiles]);
    ...
    const handleUpdateLogistics = async (freq: string, days: string) => {
        if (!selectedPoint) return;
        try {
            const { error } = await supabase
                .from('sales_points')
                .update({ delivery_frequency: freq, preferred_days: days })
                .eq('id', selectedPoint.id);
            if (error) throw error;
            setSelectedPoint({ ...selectedPoint, delivery_frequency: freq, preferred_days: days });
            setSalesPoints(salesPoints.map(p => p.id === selectedPoint.id ? { ...p, delivery_frequency: freq, preferred_days: days } : p));
        } catch (error) {
            console.error("Failed to update logistics", error);
        }
    };

    const handleUpdateRestockDetails = async (saleId: string | number, quantity: number, unitValue: number, currentInventory: number, salesSinceLastVisit: number, notes: string, status: string, paymentStatus: 'PAGO' | 'ABERTO') => {
        try {
            const { error } = await supabase
                .from('sales')
                .update({
                    ...
                    sales_since_last_visit: salesSinceLastVisit,
                    notes,
                    status,
            ...
            console.error("Failed to update restock details", error);
        }
    };
    ...
    const handleAddPoint = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const delivery_frequency = formData.get('delivery_frequency') as string;
        const preferred_days = formData.get('preferred_days') as string;

        try {
            const { error } = await supabase.from('sales_points').insert([{ name, phone, delivery_frequency, preferred_days }]);
            if (error) throw error;
            setActiveModal('none');
            fetchInitialData();
        } catch (error: any) {
            console.error("Failed to add point", error);
            alert("Erro ao cadastrar ponto de venda: " + (error.message || "Verifique sua conexÃ£o"));
        }
    };
    ...
    const handleAddSale = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            alert("Erro ao cadastrar venda: " + (error.message || "Verifique sua conexÃ£o"));
        }
    };
    ...
    const handleQuickRestock = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            alert("Erro ao reabastecer: " + (error.message || "Verifique sua conexÃ£o"));
        }
    };
    ...
    const handleAddExpense = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            alert("Erro ao cadastrar despesa: " + (error.message || "Verifique sua conexÃ£o"));
        }
    };
    ...
    const deleteSalesPoint = async (e: React.MouseEvent, id: string | number) => {
        e.stopPropagation();
        if (!confirm('Tem certeza que deseja excluir este ponto de venda? Todas as vendas vinculadas tambÃ©m serÃ£o excluÃ­das.')) return;
        setIsLoading(true);
        try {
    ...
    const sendRestockMessage = (sale: SaleEntry) => {
        const message = `OlÃ¡! Notei que os brownies entregues em ${formatDate(sale.delivery_date)} estÃ£o prÃ³ximos do vencimento (${formatDate(sale.due_date)}). Gostaria de agendar um reabastecimento para garantir produtos sempre fresquinhos?`;
        ...
        if (phone) {
            ...
            alert('Telefone nÃ£o cadastrado para este ponto de venda.');
        }
    };
            ...
            <div className="space-y-6 lg:space-y-8">
                <header className="flex flex-col lg:flex-row lg:justify-between lg:items-end gap-4 overflow-x-auto pb-2 lg:pb-0 no-scrollbar">
                    <div>
                        ...
                        <p className="text-slate-500 text-sm mt-1">VisÃ£o geral do seu negÃ³cio.</p>
                    </div>
                    <div className="flex items-center gap-2 lg:gap-3 flex-nowrap min-w-max pr-4 lg:pr-0">
                        ...
                        <button
                            ...
                            className="flex items-center gap-2 px-4 py-2 bg-slate-800 text-white border border-slate-800 rounded-xl text-sm font-medium hover:bg-slate-900 transition-colors shadow-lg shadow-slate-900/20"
                        >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
                    {[
                        ...
                        { label: 'Saldo LÃ­quido', value: stats.balance, icon: Receipt, color: 'text-slate-900', bg: 'bg-slate-100', clickable: false, format: 'currency' },
                    ].map((stat, i) => (
                        <motion.div
                            key={i}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="glass-card p-6">
                        <div className="flex justify-between items-center mb-6">
                            <h3 className="text-lg font-semibold">Ãšltimas Compras</h3>
                ...
                <div className="glass-card p-6">
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="text-lg font-semibold">Ranking de Performance (MÃ©dia Mensal)</h3>
                        <span className="text-xs text-slate-400">Baseado em todo o histÃ³rico</span>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-slate-50/50 border-y border-slate-100">
                                    <th className="px-4 py-3 text-xs font-semibold text-slate-500 uppercase">PosiÃ§Ã£o</th>
                                    <th className="px-4 py-3 text-xs font-semibold text-slate-500 uppercase">Ponto de Venda</th>
                                    <th className="px-4 py-3 text-xs font-semibold text-slate-500 uppercase text-center">MÃ©dia Vendas/MÃªs</th>
                                    <th className="px-4 py-3 text-xs font-semibold text-slate-500 uppercase text-right">MÃ©dia Faturamento/MÃªs</th>
                                </tr>
                            </thead>
        ...
        return (
            <div className="space-y-6 lg:space-y-8">
                        ...
                        >
                            ...
                            <div className="mt-4 pt-4 border-t border-slate-100 flex justify-between items-center">
                                ...
                                <div className="text-right">
                                    <p className="text-xs text-slate-400 uppercase tracking-wider font-semibold">AÃ§Ãµes</p>
                ...
                {(() => {
                    ...
                    for (let i = 0; i < startWeekday; i++) daysList.push(null);
                    for (let d = 1; d <= daysInMonth; d++) daysList.push(d);

                    const monthNames = ['Janeiro', 'Fevereiro', 'Março', 'Abril', 'Maio', 'Junho', 'Julho', 'Agosto', 'Setembro', 'Outubro', 'Novembro', 'Dezembro'];

                    // Build per-day revenue map from allSales
                    const dayRevMap: Record<string, { total: number; points: Set<string> }> = {};
                    allSales.forEach(s => {
                        if (!s.delivery_date) return;
                        const [sy, sm] = s.delivery_date.split('-');
                        if (Number(sy) !== year || Number(sm) - 1 !== month) return;
                        if (!dayRevMap[s.delivery_date]) dayRevMap[s.delivery_date] = { total: 0, points: new Set() };
                        dayRevMap[s.delivery_date].total += s.total_value;
                        dayRevMap[s.delivery_date].points.add(String(s.sales_point_id));
                    });

                    const maxRev = Math.max(...Object.values(dayRevMap).map(v => v.total), 1);
                        ...
                        <div className="glass-card p-6 space-y-6">
                            ...
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-xs text-slate-400 mt-0.5">Receita bruta prevista por dia com base nas entregas agendadas.</p>
                                </div>
                                <div className="flex items-center gap-2">
                                    <button
                                        onClick={() => setCalendarDate(new Date(year, month - 1, 1))}
                                        className="p-2 hover:bg-slate-100 border border-slate-200 rounded-xl transition-colors text-slate-600"
                                    >
                                        <ChevronRight className="w-4 h-4 rotate-180" />
                                    </button>
                                    <span className="text-sm font-bold text-slate-700 min-w-[130px] text-center">{monthNames[month]} {year}</span>
                                    <button
                                        onClick={() => setCalendarDate(new Date(year, month + 1, 1))}
                                        className="p-2 hover:bg-slate-100 border border-slate-200 rounded-xl transition-colors text-slate-600"
                                    >
                            ...
                            {/* Month summary pills */}
                            {Object.keys(dayRevMap).length > 0 && (() => {
                                const monthTotal = Object.values(dayRevMap).reduce((a, v) => a + v.total, 0);
                                const monthPoints = new Set(Object.values(dayRevMap).flatMap(v => [...v.points])).size;
                                const bestDay = Object.entries(dayRevMap).sort((a, b) => b[1].total - a[1].total)[0];
                                const [bY, bM, bD] = bestDay[0].split('-');
                                return (
                                    <div className="flex flex-wrap gap-3">
                                        <div className="flex items-center gap-2 bg-brand-50 border border-brand-100 rounded-xl px-3 py-2">
                                            <TrendingUp className="w-4 h-4 text-brand-500" />
                                            <div>
                                                <p className="text-[10px] font-semibold text-slate-400 uppercase">Total do Mês</p>
                                                <p className="text-sm font-bold text-brand-600">{formatCurrency(monthTotal)}</p>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-2 bg-emerald-50 border border-emerald-100 rounded-xl px-3 py-2">
                                            <CheckCircle2 className="w-4 h-4 text-emerald-500" />
                                            <div>
                                                <p className="text-[10px] font-semibold text-slate-400 uppercase">Pontos Ativos</p>
                                                <p className="text-sm font-bold text-emerald-600">{monthPoints} ponto{monthPoints !== 1 ? 's' : ''}</p>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-2 bg-amber-50 border border-amber-100 rounded-xl px-3 py-2">
                                            <ArrowUpRight className="w-4 h-4 text-amber-500" />
                                            <div>
                                                <p className="text-[10px] font-semibold text-slate-400 uppercase">Melhor Dia</p>
                                                <p className="text-sm font-bold text-amber-600">{bD}/{bM} · {formatCurrency(bestDay[1].total)}</p>
                                            </div>
                                        </div>
                                    </div>
                                );
                            ...
                            <div className="grid grid-cols-7 gap-1.5">
                                ...
                                {daysList.map((day, idx) => {
                                    if (day === null) {
                                        return <div key={`e-${idx}`} className="aspect-square rounded-2xl bg-slate-50/30 border border-slate-100/50" />;
                                    }
                                    const dStr = `${year}-${String(month + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
                                    const dayData = dayRevMap[dStr];
                                    const isToday = dStr === todayStr;
                                    const intensity = dayData ? dayData.total / maxRev : 0;

                                    // Color: gradient from white to brand, stronger = more revenue
                                    const bgStyle = dayData
                                        ? { background: `rgba(14, 165, 233, ${0.08 + intensity * 0.55})` }
                                        : undefined;

                                    return (
                                        <button
                                            key={dStr}
                                            onClick={() => setSelectedCalendarDate(selectedCalendarDate === dStr ? null : dStr)}
                                            className={cn(
                                                "aspect-square rounded-2xl border flex flex-col items-center justify-center gap-0.5 relative transition-all group",
                                                dayData
                                                    ? "border-brand-200 hover:border-brand-400 hover:shadow-md hover:shadow-brand-500/10 cursor-pointer"
                                                    : "border-slate-100 bg-white/80 cursor-default",
                                                isToday && "ring-2 ring-brand-500 ring-offset-1",
                                                selectedCalendarDate === dStr && "ring-2 ring-brand-600 ring-offset-2 shadow-lg"
                                            )}
                                            style={bgStyle}
                                        >
                                            <span className={cn(
                                                "text-xs font-bold leading-none",
                                                dayData ? "text-sky-900" : "text-slate-400",
                                                isToday && "text-brand-600"
                                            )}>
                                                {day}
                                            </span>
                                            {dayData && (
                                                <span className="text-[8px] md:text-[9px] font-bold text-sky-800/80 leading-none px-1 text-center">
                                                    {formatCurrency(dayData.total).replace('R$\u00a0', 'R$')}
                                                </span>
                                            )}
                                        </button>
                                    );
                            ...
                            {selectedCalendarDate && dayRevMap[selectedCalendarDate] && (() => {
                                const [, , selD] = selectedCalendarDate.split('-');
                                const daySales = allSales.filter(s => s.delivery_date === selectedCalendarDate);
                                return (
                                    <div className="border-t border-slate-100 pt-5 space-y-3 animate-fadeIn">
                                        <div className="flex items-center justify-between">
                                            <h3 className="text-sm font-bold text-slate-700">
                                                Dia {Number(selD)} de {monthNames[month]} — {formatCurrency(dayRevMap[selectedCalendarDate].total)}
                                            </h3>
                                            <button
                                        ...
                                        <div className="space-y-2">
                                            {daySales.map(s => (
                                                <div key={s.id} className="flex items-center justify-between bg-slate-50 border border-slate-100 rounded-xl px-4 py-2.5">
                                                    <div>
                                                        <p className="text-sm font-semibold text-slate-700">{s.point_name}</p>
                                                        <p className="text-xs text-slate-400">{s.product_name} · {s.quantity} un</p>
                                                    </div>
                                                    <div className="text-right">
                                                        <p className="text-sm font-bold text-brand-600">{formatCurrency(s.total_value)}</p>
                                                        <span className={cn(
                                                            "text-[10px] font-bold px-2 py-0.5 rounded-full",
                                                            s.payment_status === 'PAGO' ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-600"
                                                        )}>
                            ...
                            <div className="flex flex-wrap items-center gap-4 pt-2 border-t border-slate-100">
                                <div className="flex items-center gap-2 text-xs text-slate-400">
                                    <span className="w-3 h-3 rounded-md" style={{ background: 'rgba(14,165,233,0.12)' }} />
                                    Receita baixa
                                </div>
                                <div className="flex items-center gap-2 text-xs text-slate-400">
                                    <span className="w-3 h-3 rounded-md" style={{ background: 'rgba(14,165,233,0.50)' }} />
                                    Receita média
                                </div>
                                <div className="flex items-center gap-2 text-xs text-slate-400">
                                    <span className="w-3 h-3 rounded-md" style={{ background: 'rgba(14,165,233,0.90)' }} />
                                    Receita alta
                                </div>
                                <div className="flex items-center gap-2 text-xs text-slate-400 ml-auto">
                                    <span className="w-4 h-4 rounded-full ring-2 ring-brand-500 inline-block" />
                                    Hoje
                                </div>
                            </div>
                        </div>
            ...
            <div className="space-y-8">
                <header className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-slate-500 mt-1">HistÃ³rico de entregas e pagamentos.</p>
                    </div>
                </header>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="glass-card p-6 border-l-4 border-l-amber-500">
                        <p className="text-sm font-medium text-slate-500">Ã€ Receber</p>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-slate-50/50 border-y border-slate-100">
                                    ...
                                    <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">FabricaÃ§Ã£o</th>
                                    ...
                                    <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">AÃ§Ãµes</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100">
                                {pointSales.map((sale) => (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-slate-50/50 border-y border-slate-100">
                                    ...
                                    <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Produto/ServiÃ§o</th>
                                    ...
                                    <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-right">AÃ§Ãµes</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="glass-card p-6 bg-brand-500 text-white border-none">
                            <h3 className="font-bold mb-2">Dica de GestÃ£o</h3>
                            <p className="text-sm text-brand-50 opacity-90 leading-relaxed">
                                Mantenha suas notas fiscais organizadas para facilitar o fechamento do mÃªs.
                            </p>
                        </div>
                    </div>
                </div>
    ...
    const renderReports = () => {
        return (
            <div className="space-y-6 lg:space-y-8">
                <header className="flex flex-col lg:flex-row lg:justify-between lg:items-end gap-4 overflow-x-auto pb-2 lg:pb-0 no-scrollbar">
                    <div>
                        <h1 className="text-2xl lg:text-3xl font-bold tracking-tight">HistÃ³rico de Abastecimentos</h1>
                        <p className="text-slate-500 text-sm mt-1">Visualize e filtre todo o histÃ³rico de abastecimentos.</p>
                    </div>
                </header>
                ...
                <div className="flex flex-col lg:flex-row items-stretch lg:items-end gap-4 glass-card p-4 lg:p-6 bg-white overflow-hidden">
                    <div className="flex-1 min-w-[200px]">
                        <label className="block text-sm font-medium text-slate-700 mb-1">MÃªs de ReferÃªncia</label>
                        <input
                            type="month"
                ...
                <div className="glass-card overflow-hidden">
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-slate-50/50 border-y border-slate-100">
                                    <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">MÃªs Ref.</th>
                ...
                >
                    <div className="p-6 border-b border-slate-100 flex justify-between items-center">
                        <h3 className="text-xl font-bold">
                            ...
                            {activeModal === 'quick-restock' && 'Reabastecimento RÃ¡pido'}
                            {activeModal === 'payment-details' && (paymentDetailsType === 'PAGO' ? 'Pontos que jÃ¡ pagaram' : 'Pontos com pagamentos pendentes')}
                        </h3>
                    ...
                    >
                            ...
                            return (
                                <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2 no-scrollbar">
                                    {list.length > 0 ? (
                                        ...
                                        <p className="text-center text-slate-400 py-8 italic">Nenhum registro encontrado para este perÃ­odo.</p>
                                    )}
                                </div>
                            );
                        })()}
                        ...
                        {activeModal === 'new-point' && (
                            <div className="space-y-4">
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 mb-1">WhatsApp (DDD + NÃºmero)</label>
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 mb-1">FrequÃªncia de Abastecimento</label>
                                    <select name="delivery_frequency" className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand-500/20 outline-none">
                                        <option value="semanal">Semanal (7 dias)</option>
                                        <option value="quinzenal">Quinzenal (14 dias)</option>
                                    </select>
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 mb-1">Dia de Rota Preferencial</label>
                                    <select name="preferred_days" className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand-500/20 outline-none">
                                        <option value="terca">TerÃ§a-feira</option>
                                        <option value="quinta">Quinta-feira</option>
                                        <option value="terca,quinta">TerÃ§a e Quinta-feira</option>
                                    </select>
                                </div>
                            </div>
                        )}
                        ...
                        {activeModal === 'new-sale' && (
                            <>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-slate-700 mb-1">Valor UnitÃ¡rio</label>
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 mb-1">Data de FabricaÃ§Ã£o</label>
                        ...
                        {activeModal === 'expiring-alerts' && (
                            <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
                                <p className="text-sm text-slate-500 mb-4">Os seguintes pontos possuem produtos que vencem nos prÃ³ximos 3 dias:</p>
    ...
    const renderPricing = () => {
        ...
        const recipeYield = activeProfile.recipeYield || 1;
        const unitCMV = cmv / recipeYield;

        const suggestedPrice = unitCMV * (1 + (activeProfile.profitMarginPct || 0) / 100);
        const contributionMargin = (activeProfile.salePrice || 0) - unitCMV;
        ...
        const markup = unitCMV > 0 ? (activeProfile.salePrice || 0) / unitCMV : 0;
        const fixedCostPerUnit = (activeProfile.salesExpectation || 0) > 0 ? (activeProfile.fixedCosts || 0) / activeProfile.salesExpectation : 0;
                    ...
                    <table className="w-full text-left min-w-[800px]">
                        <thead>
                            <tr className="bg-slate-50 border-b border-slate-100 text-xs font-semibold text-slate-500 uppercase">
                                ...
                                <th className="px-4 py-2 w-28 text-center" title="PreÃ§o pago pela embalagem fechada">R$ Pacote</th>
        ...
        return (
            <div className="space-y-6 lg:space-y-8 pb-12">
                <header className="flex flex-col lg:flex-row lg:justify-between lg:items-end gap-4">
                    <div>
                        <h1 className="text-2xl lg:text-3xl font-bold tracking-tight">GestÃ£o de PrecificaÃ§Ã£o</h1>
                        <p className="text-slate-500 text-sm mt-1">ComposiÃ§Ã£o detalhada de custos por produto para cÃ¡lculo de margens e preÃ§o final.</p>
                    </div>
                </header>

                <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                    <div className="glass-card p-6 lg:col-span-1 space-y-4">
                        <h2 className="text-lg font-bold text-slate-800">Seus Produtos</h2>
                        <div className="space-y-2">
                    ...
                    <div className="lg:col-span-3 space-y-6">
                        <div className="glass-card p-6 border-t-4 border-t-brand-500">
                            ...
                            {renderItemTable('ProduÃ§Ã£o', 'productionItems', activeProfile.productionItems || [], totalProduction, 'bg-blue-50/80')}
                            {renderItemTable('Embalagem', 'packagingItems', activeProfile.packagingItems || [], totalPackaging, 'bg-emerald-50/80')}
                            {renderItemTable('Tempo / MÃ£o de Obra', 'timeItems', activeProfile.timeItems || [], totalTime, 'bg-amber-50/80')}
                            ...
                            <div className="bg-slate-900 rounded-xl p-6 text-white mb-8 shadow-xl shadow-slate-900/10 relative overflow-hidden">
                                ...
                                <h3 className="font-bold text-slate-300 uppercase tracking-wider text-sm mb-6 flex items-center gap-2 relative z-10">
                                    <Calculator className="w-4 h-4" /> Resumo e FormaÃ§Ã£o de PreÃ§o
                                </h3>

                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 relative z-10">
                                    <div className="bg-slate-800/50 p-3 rounded-lg border border-slate-700/50">
                                        <p className="text-xs font-medium text-slate-400 mb-1 uppercase tracking-wider">Custo Total Receita</p>
                                        <p className="text-xl font-bold text-white">{formatCurrency(cmv)}</p>
                                    </div>
                                    <div>
                                        <p className="text-sm font-medium text-slate-400 mb-1">Rendimento (unidades)</p>
                                        <div className="relative">
                                            <input
                                                type="number"
                                                min="1"
                                                value={activeProfile.recipeYield || 1}
                                                onChange={(e) => updateProfileNumber('recipeYield', e.target.value)}
                                    ...
                                    <div className="bg-brand-500/10 p-3 rounded-lg border border-brand-500/20">
                                        <p className="text-xs font-medium text-brand-300 mb-1 uppercase tracking-wider">Custo por Unidade</p>
                                        <p className="text-2xl font-bold text-brand-400">{formatCurrency(unitCMV)}</p>
                                    </div>
                                    <div>
                                    ...
                                    <div className="bg-brand-500/20 p-3 rounded-lg border border-brand-500/30">
                                        <p className="text-xs font-medium text-brand-200 mb-1 uppercase">PreÃ§o a Ser Cobrado</p>
                                    ...
                                    <div>
                                        <p className="text-sm font-medium text-emerald-400 mb-1 flex items-center gap-1">
                                            PreÃ§o Final Adotado <Info className="w-3 h-3" />
                                        </p>
                                        <input
                                            type="number"
                                            step="0.01"
                                            ...
                                            title="Este Ã© o preÃ§o que vocÃª realmente vai cobrar do cliente"
                                        />
                                    </div>
                                </div>
                            </div>

                            <h3 className="text-lg font-bold text-slate-800 mb-4 mt-12">AnÃ¡lise Financeira e Viabilidade do Produto</h3>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                <div className="space-y-4">
                                    <div>
                                        <label className="block text-sm font-medium text-slate-700 mb-1">Expectativa de Vendas (unidades/mÃªs)</label>
                                        <p className="text-xs text-slate-500 mb-1">Quantos <strong>{activeProfile.name}</strong> vocÃª espera vender por mÃªs?</p>
                                ...
                                <div className="space-y-4">
                                    <div className="glass-card p-4 bg-indigo-50/50">
                                        <p className="text-sm font-medium text-slate-500 mb-1">Ponto de EquilÃ­brio (Break-even)</p>
                                        <p className="text-2xl font-bold text-slate-800">{Math.ceil(breakEven)} unidades</p>
                                        <p className="text-[11px] text-slate-500 mt-1 leading-relaxed">
                                            Vendas necessÃ¡rias neste mÃªs apenas para pagar as Despesas Fixas (ficar no zero a zero). A partir disso, vocÃª comeÃ§a a ter lucro lÃ­quido.
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="glass-card p-6 border-l-4 border-blue-500">
                                    ...
                                    <p className="text-sm font-medium text-blue-700/70">{contributionMarginPct.toFixed(1)}% do preÃ§o final adotado</p>
                                    <p className="text-[11px] text-slate-500 mt-3 leading-relaxed">
                                        <strong>O que Ã©:</strong> O valor que sobra de cada venda (PreÃ§o menos Total de Materiais). Serve para pagar as despesas fixas e o seu lucro.
                                    </p>
                                </div>

                                <div className={cn("glass-card p-6 border-l-4", netProfit > 0 ? "border-emerald-500" : "border-red-500")}>
                                    <p className="text-sm font-medium text-slate-500 mb-1 flex items-center gap-2">
                                        Lucro LÃ­quido Real por Unidade
                                        <Info className="w-4 h-4" />
                                    </p>
                                    ...
                                    <p className="text-[11px] text-slate-500 mt-3 leading-relaxed">
                                        <strong>O que Ã©:</strong> O dinheiro livre que vai para o seu bolso, apÃ³s subtrair os materiais e a parcela ideal de despesas fixas para essa unidade.
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="glass-card p-6 border-l-4 border-brand-500 mt-6">
                                <h3 className="font-bold flex items-center gap-2 mb-4 text-brand-800">
                                    DistribuiÃ§Ã£o do PreÃ§o Final
                                </h3>
                                <div className="flex w-full h-4 rounded-full overflow-hidden bg-slate-100 mb-4">
                                    <div style={{ width: `${(unitCMV / (activeProfile.salePrice || 1)) * 100}%` }} className="bg-rose-400" title="Custo UnitÃ¡rio (ProduÃ§Ã£o + Embalagem + Tempo)"></div>
                                    ...
                                    <div style={{ width: `${(Math.max(0, netProfit) / (activeProfile.salePrice || 1)) * 100}%` }} className="bg-emerald-400" title="Lucro LÃ­quido"></div>
                                </div>
                                <div className="flex flex-wrap gap-4 text-xs font-medium text-slate-600">
                                    <div className="flex items-center gap-1"><span className="w-3 h-3 rounded-full bg-rose-400"></span> Custo UnitÃ¡rio ({(unitCMV / (activeProfile.salePrice || 1) * 100).toFixed(1)}%)</div>
                                    ...
                                    <div className="flex items-center gap-1"><span className="w-3 h-3 rounded-full bg-emerald-400"></span> Lucro LÃ­quido ({(Math.max(0, netProfitPct)).toFixed(1)}%)</div>
                                </div>
                            </div>
    ...
    return (
        <>
        <div className="flex min-h-screen bg-slate-50 relative print:hidden">
            {renderModal()}
            ...
            <aside className={cn(
                ...
                <div className="flex items-center justify-between lg:justify-start gap-3 px-2">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-xl font-bold tracking-tight">Beik Brownies & Cookies</span>
                    </div>
                    <button
                        onClick={() => setIsSidebarOpen(false)}
                ...
                <nav className="flex-1 space-y-2">
                    {[
                        ...
                        { id: 'reports', label: 'HistÃ³rico', icon: Filter },
            ...
            <div className="flex-1 flex flex-col min-h-screen">
                ...
                <header className="lg:hidden bg-white border-b border-slate-200 p-4 sticky top-0 z-30 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-slate-900 truncate max-w-[200px]">Beik Brownies & Cookies</span>
                    </div>
                    <button
        ...
        {/* Print Layout for Production Order */}
        <div className="hidden print:block bg-white min-h-screen">
            {productionOrders.length > 0 && Object.entries(productionOrders[0].points).map(([pointName, sales]: any, idx) => {
                const totalVencer = sales.reduce((acc: number, s: any) => acc + s.quantity, 0);
                return (
                    <div key={idx} style={{ pageBreakAfter: 'always' }} className="p-10 font-sans text-black">
                        <div className="border-b-2 border-black pb-4 mb-6 flex justify-between items-end">
                            <div>
                                <h1 className="text-3xl font-bold uppercase tracking-tight">{pointName}</h1>
                                <p className="text-sm font-semibold mt-1">Ordem de Produção e Reposição</p>
                            </div>
                            <div className="text-right">
                                <p className="text-xs uppercase font-bold text-gray-500">Data de Emissão</p>
                                <p className="text-lg font-bold">{formatDate(productionOrders[0].date.split('T')[0])}</p>
                            </div>
                        </div>

                        <div className="mb-8">
                            <h2 className="text-lg font-bold bg-gray-100 p-2 uppercase border border-black mb-4">Produtos Próximos do Vencimento</h2>
                            <table className="w-full text-left border-collapse border border-black">
                                <thead>
                                    <tr className="bg-gray-100 border-b border-black">
                                        <th className="p-3 border-r border-black font-bold uppercase text-sm">Produto</th>
                                        <th className="p-3 border-r border-black font-bold uppercase text-sm">Vencimento</th>
                                        <th className="p-3 font-bold uppercase text-sm text-center">Qtd. a Vencer</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {sales.map((sale: any) => (
                                        <tr key={sale.id} className="border-b border-black">
                                            <td className="p-3 border-r border-black font-semibold">{sale.product_name || 'Brownie'}</td>
                                            <td className="p-3 border-r border-black">{formatDate(sale.due_date)}</td>
                                            <td className="p-3 text-center font-bold">{sale.quantity}</td>
                                        </tr>
                                    ))}
                                    <tr className="bg-gray-50">
                                        <td colSpan={2} className="p-3 border-r border-black font-bold text-right uppercase">Total de Produtos a Vencer:</td>
                                        <td className="p-3 text-center font-bold text-lg">{totalVencer}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                        ...
                        <div>
                            <h2 className="text-lg font-bold bg-gray-100 p-2 uppercase border border-black mb-4">Preenchimento (Controle de Reposição)</h2>
                            <div className="grid grid-cols-2 gap-4 mb-4">
                                <div className="border border-black p-3 h-16 flex flex-col justify-between">
                                    <span className="text-xs uppercase font-bold text-gray-600">Data da Produção</span>
                                    <div className="border-b border-dotted border-black w-full mt-2 flex-1"></div>
                                </div>
                                <div className="border border-black p-3 h-16 flex flex-col justify-between">
                                    <span className="text-xs uppercase font-bold text-gray-600">Quantidade Produzida</span>
                                    <div className="border-b border-dotted border-black w-full mt-2 flex-1"></div>
                                </div>
                                <div className="border border-black p-3 h-16 flex flex-col justify-between">
                                    <span className="text-xs uppercase font-bold text-gray-600">Quantidade Entregue</span>
                                    <div className="border-b border-dotted border-black w-full mt-2 flex-1"></div>
                                </div>
                                <div className="border border-black p-3 h-16 flex flex-col justify-between">
                                    <span className="text-xs uppercase font-bold text-gray-600">Quantidade Retirada (Vencidos)</span>
                                    <div className="border-b border-dotted border-black w-full mt-2 flex-1"></div>
                                </div>
                                <div className="border border-black p-3 h-16 flex flex-col justify-between">
                                    <span className="text-xs uppercase font-bold text-gray-600">Pagamento Recebido? (Sim/Não)</span>
                                    <div className="border-b border-dotted border-black w-full mt-2 flex-1"></div>
                                </div>
                                <div className="border border-black p-3 h-16 flex flex-col justify-between">
                                    <span className="text-xs uppercase font-bold text-gray-600">Valor Recebido (R$)</span>
                                    <div className="border-b border-dotted border-black w-full mt-2 flex-1"></div>
                                </div>
                            </div>
                            <div className="border border-black p-3 h-32 flex flex-col">
                                <span className="text-xs uppercase font-bold text-gray-600">Observações</span>
                                <div className="border-b border-dotted border-black w-full mt-6"></div>
                                <div className="border-b border-dotted border-black w-full mt-6"></div>
                                <div className="border-b border-dotted border-black w-full mt-6"></div>
                            </div>
                        </div>
